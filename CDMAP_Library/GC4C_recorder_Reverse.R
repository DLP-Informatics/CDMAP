...
{
  increment <- as.integer(Rev_GC3C_Right[rowiter,coliter])
  increment <- increment + 1
  Rev_GC3C_Right[rowiter, coliter] <- increment 
  
}
if(orientation == '+' && flagcheck == 'Left')
{
  increment <- as.integer(GC3C_Left[rowiter,coliter])
  increment <- increment + 1
  GC3C_Left[rowiter, coliter] <- increment 
